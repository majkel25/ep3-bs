...
/**
 * POST /api/ssa/v1/admin/policy-unpublish.php
 *
 * Unpublish a policy (set status = unpublished). Admin/Owner only.
 *
 * Body (JSON):
 *   id int required
 */
...
try {
    ...
    $pdo->prepare(
        "UPDATE ssa_policies SET status = 'unpublished' WHERE id = :id"
    )->execute(['id' => $policyId]);

    ssaApiPolicyAudit($pdo, $policyId, $existing['current_version_id'] ? (int)$existing['current_version_id'] : null, 'unpublished', [
        'status' => $existing['status'],
    ], ['status' => 'unpublished'], $callerUid);
    ...
    error_log('SSA admin/policy-unpublish.php failed: ' . $e->getMessage());
    ssaApiJsonResponse(500, ['error' => 'server_error', 'message' => 'Unable to unpublish policy.']);
}

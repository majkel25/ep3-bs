...
try {
    ...
    $sample = $pdo->query("SELECT u.email, u.name, um.id, um.plan_id, um.status, um.started_at, p.plan_key FROM ssa_user_memberships um JOIN bs_users u ON um.uid=u.id JOIN ssa_membership_plans p ON um.plan_id=p.id WHERE um.source='legacy_excel_import' LIMIT 1")->fetch(PDO::FETCH_ASSOC);
